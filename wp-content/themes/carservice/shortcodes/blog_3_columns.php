<?php
//blog
function cs_theme_blog_3_columns($atts, $content)
{
		...
		while ($blog_query->have_posts()) : $blog_query->the_post();
			...
			if($layout_type=="row")
			{
				if($i==0 || $i%3==0)
				{
					...
					$output .= '<ul class="blog three-columns clearfix">';
				}
				$output .= $post->getLiCssClass("vc_col-sm-4 wpb_column vc_column_container");
			}
			else
			{
				if($i==0 || ($i%$rows==0 || $rows==1))
				{
					if(($i%$rows==0 || $rows==1) && $i>0)
					{
						$output .= '</ul></div>';
					}
					$output .= '<div class="vc_col-sm-4 wpb_column vc_column_container"><ul class="blog three-columns clearfix">';
				}
				$output .= $post->getLiCssClass();
			}
			$output .= $post->getThumbnail("big-thumb");
			$output .= $post->getPostDetails();
			if((int)$show_post_title)
			{
				$output .= '<h4 class="box-header align-left"><a href="' . esc_url(get_permalink()) . '" title="' . esc_attr(get_the_title()) . '">' . get_the_title() . '</a></h4>';
			}
			if((int)$show_post_excerpt)
			{
				$output .= apply_filters('the_excerpt', ((int)$excerpt_length ? substr(get_the_excerpt(), 0, strpos(get_the_excerpt(), ' ', (strlen(get_the_excerpt())<(int)$excerpt_length ? strlen(get_the_excerpt()) : (int)$excerpt_length))) : get_the_excerpt()));
			}
			if((int)$read_more)
			{
...
//visual composer
function cs_theme_blog_3_columns_vc_init()
{
	//get posts list
	...
	vc_map( array(
		"name" => __("Blog 3 Columns", 'carservice'),
		"base" => "blog_3_columns",
		"class" => "",
		"controls" => "full",
		...
		"params" => array(
			array(
				"type" => "dropdown",
				"class" => "",
				"heading" => __("Layout type", 'carservice'),
				"param_name" => "layout_type",
				"value" => array(__("Rows layout", 'carservice') => "row", __("Columns layout", 'carservice') => "column"),
				"description" => __("How posts should be displayed: row by row or column by column", 'carservice')
			),
			array(
				"type" => "dropdown",
				"class" => "",
			...
			array(
				...
				"param_name" => "order_by",
				"value" => array(__("Title, menu order", 'carservice') => "title,menu_order", __("Menu order", 'carservice') => "menu_order", __("Date", 'carservice') => "date", __("Post views", 'carservice') => "views", __("Comment count", 'carservice') => "comment_count", __("Random", 'carservice') => "rand")
			),
			array(
				"type" => "dropdown",
				"class" => "",
...
add_action("init", "cs_theme_blog_3_columns_vc_init");
?>