...
function carservice_testimonials_shortcode($atts)
{
	...
	$output = '<div class="testimonials-container' . ($top_margin!="none" ? ' ' . esc_attr($top_margin) : '') . (!empty($type) ? ' type-' . esc_attr($type) : '') . ($el_class!="" ? ' ' . $el_class : '') . '">';
	...
	if(is_rtl())
	{
		for($i=$testimonials_count-1; $i>=0; $i--)
		{
			$output .= '<li>
					...
					' . (!empty($type) && $type=="small" ? '<div class="ornament' . (!empty($atts["testimonials_icon" . $i]) && $atts["testimonials_icon" . $i]!="" && $atts["testimonials_icon" . $i]!="none" ? '"><div class="hexagon small"><div class="sl-small-' . $atts["testimonials_icon" . $i] . '"></div></div>' : ' empty-circle">') . '</div>' : '') . '
