...
if(function_exists("vc_add_shortcode_param"))
{
	...
	{
		$value = explode(",", $value);
		$output = '<input type="button" value="' . esc_html__('Add list item', 'carservice') . '" name="'.esc_attr($settings['param_name']).'" class="button '.esc_attr($settings['param_name']).' '.esc_attr($settings['type']).'" style="width: auto; padding: 0 10px 1px;"/>';
		return $output;
	}
	//add item window
