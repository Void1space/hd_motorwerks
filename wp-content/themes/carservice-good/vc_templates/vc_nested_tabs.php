...
foreach ( $tab_titles as $tab ) {
	...
	if(isset($tab_matches[1][0])) {
		...
		if(substr($href, 0, 4)!="http" && substr($href, 0, 5)!="https")
			$href = "#" . $href;
		$tabs_nav .= '<li><a href="'. $href .'">' . $tab_matches[1][0] . '</a></li>';
...
if ( 'vc_tour' == $this->shortcode) {
    $output .= "\n\t\t\t" . '<div class="wpb_tour_next_prev_nav clearfix"> <span class="wpb_prev_slide"><a href="#prev" title="'.__('Previous slide', 'carservice').'">'.__('Previous slide', 'carservice').'</a></span> <span class="wpb_next_slide"><a href="#next" title="'.__('Next slide', 'carservice').'">'.__('Next slide', 'carservice').'</a></span></div>';
}
$output .= "\n\t\t".'</div> '.$this->endBlockComment('.wpb_wrapper');
$output .= "\n\t".'</div> '.$this->endBlockComment($element);*/

$output .= '<div class="clearfix tabs'.(isset($width) ? $width : '').$el_class.($top_margin!="none" ? ' ' . $top_margin : '').'" data-interval="'.$interval.'">';
$output .= "\n\t\t\t".$tabs_nav;
$output .= "\n\t\t\t".wpb_js_remove_wpautop($content);
