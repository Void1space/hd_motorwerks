...
function costCalculatorChangeSkin(event)
{
	...
	else if(jQuery(this).val()=="gymbase")
	{
		...
		jQuery("#form_label_color").val("FFFFFF").keyup().prev().css("background-color", "#" + jQuery(this).data("color"));
		jQuery("#inactive_color").val("343436").keyup().prev().css("background-color", "#" + jQuery("#inactive_color").val());
		jQuery("#primary_font_custom, #primary_font, #secondary_font_custom, #secondary_font").val("");
		if(event.type=="selectmenuchange")
		{
			jQuery("#primary_font, #secondary_font").selectmenu("refresh").trigger("selectmenuchange");
		}
