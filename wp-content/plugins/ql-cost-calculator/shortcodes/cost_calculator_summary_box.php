...
function cost_calculator_summary_box_shortcode($atts)
{
	...
	$output = '<div class="cost-calculator-box cost-calculator-summary-box cost-calculator-clearfix' . ($el_class!="" ? ' ' . esc_attr($el_class) : '') . (!empty($icon) && $icon!="none" ? ' ' . esc_attr($icon) : '') . '">
		<span class="cost-calculator-summary-price' . ($currency_size=="small" ? ' cost-calculator-small-currency' . (!empty($currency_align) ? ' cost-calculator-align-' . esc_attr($currency_align) : '') : '') . '" id="' . esc_attr($id) . '">' . ($currency_position=="before" ? '<span class="cost-calculator-currency">' . $currency . "</span>" : '') . '0.00' . ($currency_position=="after" ? '<span class="cost-calculator-currency">' . $currency . '</span>' : '') . '</span>';
	...
	vc_map( array(
		...
		"params" => array(
			...
			array(
				"type" => "dropdown",
				"class" => "",
				"heading" => __("Currency vertical align", 'cost-calculator'),
