...
echo $dbmsg;
	...
	$urltrimmed = remove_query_arg( array('taction', 'id') );
		...
		if($keystatus['ack']!='success' && $currentform->endpoint != '7'){
			...
			$actionhtml = '<a href="'.$url_tempeditbtn.'" class="rfbtn button button-secondary dashicons-before dashicons-admin-generic">'.__('Edit', 'wp-review-slider-pro').'</a> <a href="'.$url_tempdelbtn.'" class="rfbtn button button-secondary dashicons-before dashicons-trash">'.__('Delete', 'wp-review-slider-pro').'</a> <a href="'.$url_tempcopybtn.'" class="rfbtn button button-secondary dashicons-before dashicons-admin-page">'.__('Copy', 'wp-fb-reviews').'</a> <span class="rfbtn button button-primary dashicons-before dashicons-star-filled retreviewsbtn"> '.__('Get Tweets', 'wp-fb-reviews').'</span>';
		}
			
		$html .= '<tr id="'.$currentform->id.'">
				<th scope="col" class=" manage-column">'.$currentform->id.'</th>
				<th scope="col" class=" manage-column" style="min-width: 200px;"><b><span class="titlespan">'.$currentform->title.'</span></b></th>
				<th scope="col" class="tdquery manage-column">'.stripslashes($currentform->query).'</th>
				<th scope="col" class=" manage-column">'.$lastranon.'</th>
				<th scope="col" class="manage-column" limage="'.$currentform->profile_img.'" fcats="'.$fcategories.'" fposts="'.$fposts.'" ftitle="'.$currentform->title.'" epoint="'.$currentform->endpoint.'" squery="'.$currentform->query.'">'.$actionhtml.'</th>
			</tr>';
	}
	} else {
