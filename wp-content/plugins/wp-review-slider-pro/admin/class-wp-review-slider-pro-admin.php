...
class WP_Review_Pro_Admin {
	...
	public function enqueue_scripts() {
		...
		if(isset($_GET['page'])){
			...
			if($_GET['page']=="wp_pro-templates_posts"){
				...
				//admin js
				wp_enqueue_script('wprevpro_templates_posts_page-js', plugin_dir_url( __FILE__ ) . 'js/wprevpro_templates_posts_page.js', array( 'jquery','wp-color-picker' ), $this->version, false );
				//used for ajax
				...
				wp_register_script( 'wp-color-picker-alpha3', plugin_dir_url( __FILE__ ) . 'js/wprevpro-wp-color-picker-alpha.js', array( 'wp-color-picker' ), '3.0.1', false );
				...
				/*
				wp_enqueue_script( $this->_token."_plublic-min", plugin_dir_url(dirname(__FILE__)). 'public/js/wprev-public.min.js', array( 'jquery' ), $this->version, true );
				//used for ajax
			...
			//$typearray = unserialize(WPREV_TYPE_ARRAY);
				...
				wp_localize_script('wprevpro_badges_page-js', 'adminjs_script_vars', 
					...
					'popupmsg3' => __('Or you can add the following php code to your theme:', 'wp-review-slider-pro'),
	...
	public function wppro_woo_settings_init()
	{
		...
		add_settings_field("woo_radio_sync", __( 'Sync WooCommerce:', 'wp-review-slider-pro' ), array($this,'woo_radio_display_sync'), "wp_pro-get_woo", "wprevpro_woo_section_developers",
			[
		...
		add_settings_field(
			'woo_sync_all', 
			__( 'Sync Type:', 'wp-review-slider-pro' ),
	...
	public function woo_radio_display_sync($args)
		...
		if(!isset($options['woo_radio_sync'])){
		   ...
		   ?>
				...
				<p class="description">
				<?php
				echo __('Tells the plugin what type of comment to sync. ', 'wp-review-slider-pro');
				?>
				</p>
		   <?php
		}
