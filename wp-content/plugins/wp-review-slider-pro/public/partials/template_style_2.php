...
if(	$currentform[0]->template_type=="widget"){
	$iswidget=true;
?>
<div class="wprevpro_t2_outer_div_widget w3_wprs-row wprevprodiv">
<?php
	} else {
	$iswidget=false;
?>
...
foreach ( $rowarray[$x] as $review ) 
{
		...
		//star number --------------------------------------
		$starfile_burl_logo_array = $templateclass->wprevpro_get_star_logo_burl($review,$imgs_url,$currentform[0],"t2",$template_misc_array);
	...
	if($currentform[0]->display_num>0){
		...
		$companyhtml = $templateclass->wprevpro_get_companyhtml($review,$template_misc_array,"t2");
		...
		if($userpic==""){
			...
			$userpichtml = $profilelink['start'].'<img src="'.$userpic.'" alt="'.$tempreviewername.' Avatar" class="wpproslider_t2_IMG_2 wprevpro_avatarimg" />'.$profilelink['end'];
			if(isset($template_misc_array['avataropt'])){
				if($template_misc_array['avataropt']=='hide'){
					...
					$userpichtml = $profilelink['start'].'<img src="'.$tempuserpic.'" alt="'.$tempreviewername.' Avatar" class="wpproslider_t2_IMG_2 wprevpro_avatarimg" />'.$profilelink['end'];
				}
			}
		}
		...
		if(isset($template_misc_array['showtitle'])){
			if(isset($review->review_title)){
				if($template_misc_array['showtitle']=="hide"){
					...
					$title = '<span class="wprevpro_t2_DIV_title">'.stripslashes(strip_tags($review->review_title)).' </span>';
				}
			}
		}
		...
		$tempmasonrydiv =' wprevpro_t2_DIV_1'.$tempwidget.' w3_wprs-col l'.$perrow;
		if($ajaxsliload && $currentform[0]->createslider == "sli"){
			$tempmasonrydiv =' wprevpro_t1_DIV_1'.$tempwidget.' w3_wprs-col ';
		}
		if(	$currentform[0]->display_masonry=="yes" && $currentform[0]->createslider != "sli"){
			$tempmasonrydiv = $masonryclass_item.' wprevpro_t2_DIV_1'.$tempwidget.' ';
		}
...
<div class="wpproslider_t2_DIV_1a">
	<div class="wpproslider_t2_DIV_2 wprev_preview_bg1_T<?php echo $currentform[0]->style; ?><?php if($iswidget){echo "_widget";} ?> wprev_preview_bradius_T<?php echo $currentform[0]->style; ?><?php if($iswidget){echo "_widget";} ?>">
	<?php echo $userpichtml; ?>
	<div class="indrevdiv wpproslider_t2_DIV_3">
		<p class="wpproslider_t2_P_4 wprev_preview_tcolor1_T<?php echo $currentform[0]->style; ?><?php if($iswidget){echo "_widget";} ?>">
		...
		</p><strong class="wpproslider_t2_STRONG_5 wprev_preview_tcolor2_T<?php echo $currentform[0]->style; ?><?php if($iswidget){echo "_widget";} ?>"><?php echo $tempreviewername; ?></strong> <span class="wpproslider_t2_SPAN_6 wprev_preview_tcolor2_T<?php echo $currentform[0]->style; ?><?php if($iswidget){echo "_widget";} ?>"><?php echo $companyhtml;?><span class="wprev_showdate_T<?php echo $currentform[0]->style; ?><?php if($iswidget){echo "_widget";} ?>"><?php echo $datestring; ?></span></span>
		<?php echo $logo; ?>
	</div>
	<?php echo $media; ?>
