...
if(	$currentform[0]->template_type=="widget"){
	$iswidget=true;
?>
<div class="wprevpro_t9_outer_div_widget w3_wprs-row wprevprodiv">
<?php
	} else {
	$iswidget=false;
?>
...
foreach ( $rowarray[$x] as $review ) 
{
	//fix for slickslider more than one row loading on ajax
		...
		//star number --------------------------------------
		$starfile_burl_logo_array = $templateclass->wprevpro_get_star_logo_burl($review,$imgs_url,$currentform[0],"t9",$template_misc_array);
	...
	if($currentform[0]->display_num>0){
		...
		$companyhtml = $templateclass->wprevpro_get_companyhtml($review,$template_misc_array,"t9");
		...
		if($userpic==""){
			...
			$userpichtml = $profilelink['start'].'<img src="'.$userpic.'" alt="'.$tempreviewername.' Avatar" class="wpproslider_t9_IMG_2 wprevpro_avatarimg" />'.$profilelink['end'];
			if(isset($template_misc_array['avataropt'])){
				if($template_misc_array['avataropt']=='hide'){
					...
					$userpichtml = $profilelink['start'].'<img src="'.$tempuserpic.'" alt="'.$tempreviewername.' Avatar" class="wpproslider_t9_IMG_2 wprevpro_avatarimg" />'.$profilelink['end'];
				}
			}
		}
		...
		if(isset($template_misc_array['showtitle'])){
			if(isset($review->review_title)){
				if($template_misc_array['showtitle']=="hide"){
					...
					$title = '<span class="wprevpro_t9_DIV_title">'.stripslashes(strip_tags($review->review_title)).' </span>';
				}
			}
		}
		...
		$tempmasonrydiv =' wprevpro_t9_DIV_1'.$tempwidget.' w3_wprs-col l'.$perrow;
		if($ajaxsliload && $currentform[0]->createslider == "sli"){
			$tempmasonrydiv =' wprevpro_t1_DIV_1'.$tempwidget.' w3_wprs-col ';
		}
		if(	$currentform[0]->display_masonry=="yes" && $currentform[0]->createslider != "sli"){
			$tempmasonrydiv =$masonryclass_item.' wprevpro_t9_DIV_1'.$tempwidget.' ';
		}
		
		//starhtlm
		$starhtmlarray = $templateclass->wprevpro_get_starhtml($review,$template_misc_array,$currentform,$starfile,'yes');
		...
		if(intval($review->rating)>0){
			$ratingfive='<div class="wpprooutoffive">'.intval($review->rating).' out of 5 stars</div>';
		}
...
<div class="wpproslider_t9_DIV_1a">
	<div class="wpproslider_t9_DIV_2 wprev_preview_bg1_T<?php echo $currentform[0]->style; ?><?php if($iswidget){echo "_widget";} ?> wprev_preview_bradius_T<?php echo $currentform[0]->style; ?><?php if($iswidget){echo "_widget";} ?>">
	<div class="wpproslider_t9_DIV_2_top">
		<div class="wpproslider_t9_DIV_3L" <?php echo $tempuserpicnone; ?>><?php echo $userpichtml; ?></div>
		<div class="wpproslider_t9_star_DIV"><span class="wprevpro_star_imgs_T<?php echo $currentform[0]->style; ?><?php if($iswidget){echo "_widget";} ?>"><?php echo $starhtml; ?></span></div>
		<?php echo $ratingfive; ?>
		<div class="wpproslider_t9_DIV_3_logo"><?php echo $logo; ?></div>
	</div>
		
	<div class="wpproslider_t9_DIV_3">
			<div class="t9displayname wpproslider_t9_STRONG_5 wprev_preview_tcolor2_T<?php echo $currentform[0]->style; ?><?php if($iswidget){echo "_widget";} ?>"><?php echo $tempreviewername; ?><?php echo $companyhtml;?></div>
			<div class="wpproslider_t9_SPAN_6 wprev_preview_tcolor2_T<?php echo $currentform[0]->style; ?><?php if($iswidget){echo "_widget";} ?>"><span class="wprev_showdate_T<?php echo $currentform[0]->style; ?><?php if($iswidget){echo "_widget";} ?>"><?php echo $datestring.$viatext; ?></span></div>
	</div>

	<div class="indrevdiv wpproslider_t9_DIV_4">
			<p class="wpproslider_t9_P_4 wprev_preview_tcolor1_T<?php echo $currentform[0]->style; ?><?php if($iswidget){echo "_widget";} ?>">
