...
for ($x = 0; $x < count($rowarray); $x++) {
	...
	foreach ( $rowarray[$x] as $review ) 
	{
		...
		//star number --------------------------------------
		$starfile_burl_logo_array = $templateclass->wprevpro_get_star_logo_burl($review,$imgs_url,$currentform[0],"t4",$template_misc_array);
		...
		if(	$currentform[0]->display_masonry=="yes" && $currentform[0]->createslider != "sli"){
			$tempmasonrydiv = $masonryclass_item.' wprevpro_t4_DIV_1';
		}
