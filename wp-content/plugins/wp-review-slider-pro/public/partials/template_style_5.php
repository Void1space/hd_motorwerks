...
if(	$currentform[0]->template_type=="widget"){
	$iswidget=true;
?>
<div class="wprevpro_t5_outer_div_widget w3_wprs-row wprevprodiv">
<?php
	} else {
	$iswidget=false;
?>
...
foreach ( $rowarray[$x] as $review ) 
{
	...
	//fix for slickslider more than one row loading on ajax
		...
		//star number --------------------------------------
		$starfile_burl_logo_array = $templateclass->wprevpro_get_star_logo_burl($review,$imgs_url,$currentform[0],"t5",$template_misc_array);
	...
	if($currentform[0]->display_num>0){
		...
		$companyhtml = $templateclass->wprevpro_get_companyhtml($review,$template_misc_array,"t5");
		if($companyhtml!=''){
		...
		if($tempreviewername!='' && isset($template_misc_array['avataropt']) && $template_misc_array['avataropt']=='hide'){
			$tempreviewername = " - ".$tempreviewername;
		}
		...
		if($userpic==""){
			...
			$userpichtml = $profilelink['start'].'<img src="'.$userpic.'" alt="'.$tempreviewername.' Avatar" class="wpproslider_t5_IMG_2 wprevpro_avatarimg" />'.$profilelink['end'];
		}
		
		if(isset($template_misc_array['avataropt'])){
			if($template_misc_array['avataropt']=='hide'){
				$userpichtml = '';
				$datestring = $tempreviewername.$datestring.$companyhtml;
				$picdisplaynone = "style='display:none;'";
				...
				$userpichtml = $profilelink['start'].'<img src="'.$tempuserpic.'" alt="'.$tempreviewername.' Avatar" class="wpproslider_t5_IMG_2 wprevpro_avatarimg" />'.$profilelink['end'];
			}
		}
		...
		if(isset($template_misc_array['showtitle'])){
			if(isset($review->review_title)){
				if($template_misc_array['showtitle']=="hide"){
					...
					$title = '<span class="wprevpro_t5_DIV_title">'.stripslashes(strip_tags($review->review_title)).' </span>';
				}
			}
		}
		...
		$tempmasonrydiv =' wprevpro_t5_DIV_1'.$tempwidget.' w3_wprs-col l'.$perrow;
		...
		if(	$currentform[0]->display_masonry=="yes" && $currentform[0]->createslider != "sli"){
			$tempmasonrydiv =$masonryclass_item.' wprevpro_t5_DIV_1'.$tempwidget.' ';
		}
